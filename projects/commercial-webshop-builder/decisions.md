...
## Decision 003 — Historical decision to keep Spike04 open
...
Run 001 and Run 002 produced useful sourcing-process and intake findings. They did not produce sufficient quantified evidence for automation or a final commercial go-decision. This was the correct interim decision before formal canonical closure.
...
**Tracking:** Issue #54 established the initial dossier. Issue #57 governs the current status alignment.
...
## Decision 007 — Historical next-case rule

The initial canonical dossier defined one privacy-safe manual validation case with a maximum of 60 minutes of active sourcing. Completing that case would not automatically close Spike04.

This rule is superseded as a Spike04 closure mechanism by Decision 009. The timebox and measurement fields remain useful for the next commercial test.
...
GitHub issues, pull requests and `main` form the current project control plane.

## Decision 009 — Formally close Spike04 and continue with constraints
...
Spike04 is formally closed because its useful findings, limitations and unproven assumptions are now sufficiently documented. Closure is administrative and evidentiary; it is not a claim that profitable delivery, willingness to pay or repeatability has been proven.

The project may continue only through one bounded commercial validation action focused on the primary uncertainty: profitable delivery time and repeatable value per sourcing case.
...
## Decision 010 — Use main as the sole future canonical branch

`main` is the only future canonical branch for Classic Parts Finder. `main_n8n`, Spike branches and open draft pull requests are historical or supporting evidence only and must not be treated as future integration targets.

Verdent is not used in the current workflow. GitHub remains the source of truth.
