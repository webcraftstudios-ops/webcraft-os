...
import { formatDart } from '@/domain/darts';
          ...
          {turns.map((turn) => {
            ...
            return (
              <article className="rounded-lg border border-[var(--dl-border)] bg-[var(--dl-bg)] p-4" key={turn.id}>
                ...
                {turn.entry.mode === 'per-dart' ? (
                  <p className="mt-3 text-sm font-semibold text-[var(--dl-text)]">
                    Darts: {turn.entry.darts.map(formatDart).join(' · ')}
                  </p>
                ) : null}
                ...
                {turn.snapshotId && (
                  <div className="mt-4 border-t border-[var(--dl-border)] pt-4">
                    ...
                    {(() => {
                      const snap = state.snapshots.find((snapshot) => snapshot.id === turn.snapshotId);
                      return snap ? (
